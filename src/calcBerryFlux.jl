...
```
```math
df_{n}(\bm{k})=\frac{1}{2\pi}\mathrm{Im}\left[\left(\mathrm{Log}U_{n,1}(\bm{k})+\mathrm{Log}U_{n,2}(\bm{k}+\bm{e}_{1})-\mathrm{Log}U_{n,1}(\bm{k}+\bm{e}_{2})-\mathrm{Log}U_{n,1}(\bm{k})\right)\right]
```
